...
Add these to your `.env` file (or set them in your Expo environment):
...
The integration is ready to use. Here's how to use it:
...
```typescript
const newJob = await supabaseQueries.jobs.create(jobData, userId);
```

### Example: Update an invoice
```typescript
await supabaseQueries.invoices.update(invoiceId, { status: 'paid' });
```

## 4. Authentication

You'll need to implement Supabase authentication. Here's a basic example:
...
4. Replace AsyncStorage calls with Supabase queries
...
- The `user_id` field links data to authenticated users
